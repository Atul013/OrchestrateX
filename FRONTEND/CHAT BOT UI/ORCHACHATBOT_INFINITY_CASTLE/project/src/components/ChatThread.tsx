...
export const ChatThread: React.FC<ChatThreadProps> = ({ messages, onRefineMessage }) => {
  ...
  return (
    <div className="flex-1 overflow-y-auto p-4 md:p-6 space-y-4 md:space-y-6 custom-scrollbar relative" onScroll={handleScroll}>
      <div ref={topRef} />
      {messages.length === 0 ? (
        <div className="flex flex-col items-center justify-center h-full text-amber-300/70 select-none">
          <span className="text-3xl md:text-4xl mb-4">💬</span>
          <p className="text-lg md:text-xl font-medium">Start a new conversation...</p>
          <p className="text-sm md:text-base mt-2 text-amber-400/60">Type your message below to begin.</p>
        </div>
      ) : (
        messages.map((message, index) => (
          ...
          >
            {message.type === 'assistant' && (
              <div className="w-7 h-7 md:w-8 md:h-8 bg-gradient-to-br from-amber-500 via-orange-500 to-red-600 rounded-full flex items-center justify-center flex-shrink-0 border border-amber-600/30 shadow-lg">
            ...
            <div className={`max-w-[85%] md:max-w-[80%] ${message.type === 'user' ? 'order-1' : ''}`}>
              {message.type === 'assistant' && message.orchestrationResult ? (
                ...
                <div className={`p-3 md:p-4 rounded-2xl ${
                  message.type === 'user'
                    ? 'bg-gradient-to-r from-amber-500 via-orange-500 to-red-600 text-white ml-auto border border-amber-600/30 shadow-lg'
                    : 'bg-gradient-to-r from-amber-900/30 via-orange-900/20 to-red-950/30 backdrop-blur-xl border border-amber-600/30 text-amber-100 shadow-lg'
                }`}>
              ...
              <p className="text-xs text-amber-400/70 mt-1 md:mt-2 px-2">
                {message.timestamp.toLocaleTimeString()}
              </p>
            </div>
            {message.type === 'user' && (
              <div className="w-7 h-7 md:w-8 md:h-8 bg-gradient-to-br from-amber-700 to-orange-700 rounded-full flex items-center justify-center flex-shrink-0 border border-amber-600/30 shadow-lg">
